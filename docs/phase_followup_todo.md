...
> 최종 갱신: 2026-04-21 (ADR-018 상한가 즉시 청산 반영)
> 현 baseline: **PF 4.56** (1주 가중) / 248건 / 41종목 / Pure trailing + 고정 -8% 손절 + 돌파폭 ≥ 3% + BE3 + 상한가 청산
> 청산 분포: forced_close 54.0% / breakeven_stop 28.2% / stop_loss 10.1% / limit_up_exit 6.0% / trailing_stop 1.6%
...
### 5-a. 페이퍼 1일차 관찰 기반 구조 개선 (ADR-017/018)

- [x] Breakeven Stop BE3 (ADR-017) — PF 3.88 → 4.28, 약세 PF 2.16 → 3.11
- [x] 상한가 즉시 청산 (ADR-018) — PF 4.28 → 4.56 (+6.5%), limit_up_exit 15건 평균 +8.64%
...
- [ ] 슬리피지 실측 (현재 추정 0.03%)
- [ ] NXT 체결 섞임 여부 확인
- [ ] 상한가 도달 종목의 실제 체결 성공률 (시뮬 100% 전제, ADR-018 페이퍼 체크)
- [ ] `raise_stop_to_limit_up_floor` 발동 빈도 (체결 실패 케이스)
