"""scripts/grid_adx.py — ADX 임계값 그리드."""
import asyncio, copy, os, pickle, sqlite3, sys, yaml
...
# (name, desc, adx_enabled, adx_min)
SCENARIOS = [
    ("A15",  "ADX >= 15",         True,  15),
    ("A20",  "ADX >= 20 (현재)",  True,  20),
    ("A25",  "ADX >= 25",         True,  25),
    ("A30",  "ADX >= 30",         True,  30),
    ("Aoff", "ADX 비활성",        False, 0),
]
...
async def main():
    ...
    for name, desc, adx_en, adx_min in SCENARIOS:
        # config 복제 + ADX 변경
        d = dict(base_dict)
        d["adx_enabled"] = adx_en
        d["adx_min"] = float(adx_min)
        tcfg_dict = d
        ...
        total = len(trades)
        if total == 0:
            print(f"{name:<6} {desc:<22} | NO TRADES")
            continue
