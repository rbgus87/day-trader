"""scripts/grid_trail_range.py — trail min/max 범위 그리드."""
...
SCENARIOS = [
    ("R0", 0.02, 0.10, "min2% max10% (현재)"),
    ("R1", 0.01, 0.05, "min1% max5% (전종목 5%고정)"),
    ("R2", 0.01, 0.03, "min1% max3% (전종목 3%고정)"),
    ("R4", 0.02, 0.15, "min2% max15% (넓게)"),
]

def run_f_day(day_candles, strategy, costs, tcfg, ticker, t_min, t_max):
    ...
    def calc_trail(peak):
        if atr_pct is not None:
            return calculate_atr_trailing_stop(peak, atr_pct, TRAIL_MULT, t_min, t_max)
        return peak * 0.995
...
def run_multi(ticker, all_candles, tcfg, bcfg, tkm, mmap, t_min, t_max):
    ...
    for date, dc in df.groupby("date"):
        ...
        dt = run_f_day(dd, strat, costs, tcfg, ticker, t_min, t_max)
...
def _worker(args):
    tk, tkm, cp, tcfg, bcfg, mm, tmin, tmax = args
    return tk, run_multi(tk, pickle.loads(cp), tcfg, bcfg, tkm, mm, tmin, tmax)
...
async def main():
    ...
    print(f"Trail range grid (mult={TRAIL_MULT})")
    print(f"Tickers: {len(cc)}, Workers: {w}")
    print()
    print(f"{'name':<5} {'range':<22} | {'n':>4} {'PF':>6} {'PnL':>12} | {'sl':>4} {'ts':>4} {'fc':>4} | {'occ':>4}")
    print("-"*75)

    for name, tmin, tmax, desc in SCENARIOS:
        tasks = [(tk, tkm.get(tk,"?"), cc[tk], cfg, bcfg, mm, tmin, tmax) for tk in cc]
        trades = []
        ...
        print(f"{name:<5} {f'min{tmin*100:.1f}% max{tmax*100:.0f}%':<22} | {total:>4} {pf:>6.2f} {sum(pnl_list):>+12,.0f} | "
