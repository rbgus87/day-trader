"""scripts/grid_market_filter.py — 시장 필터 MA 길이 그리드."""
...
# (name, desc, market_filter_enabled, ma_length)
SCENARIOS = [
    ("Moff", "비활성",         False, 5),
    ("M3",   "MA 3일",        True,  3),
    ("M5",   "MA 5일 (현재)",  True,  5),
...
def run_multi(ticker, all_candles, tcfg, bcfg, tkm, mmap, mf_enabled):
    ...
    for date, dc in df.groupby("date"):
        ...
        skip = False
        # 시장 필터 (시나리오별 on/off)
        if mf_enabled and tkm in ("kospi","kosdaq"):
...
def _worker(args):
    tk, tkm, cp, tcfg_dict, bcfg, mm, mf_enabled = args
    tcfg = TradingConfig(**tcfg_dict)
    return tk, run_multi(tk, pickle.loads(cp), tcfg, bcfg, tkm, mm, mf_enabled)
...
async def main():
    ...
    print(f"Market filter grid")
    print(f"Tickers: {len(cc)}, Workers: {w}")
    print()
    hdr = f"{'name':<6} {'desc':<16} | {'n':>4} {'PF':>6} {'PnL':>12} {'per':>8} | {'sl':>4} {'ts':>4} {'fc':>4} | {'occ':>4} {'DD':>10} | {'bull':>5} {'side':>5} {'bear':>5}"
    print(hdr)
    print("-"*len(hdr))

    for name, desc, mf_enabled, ma_len in SCENARIOS:
        # 시장 강세 맵 생성 (MA 길이별)
        if mf_enabled:
            mm = build_market_strong_by_date(app.db_path, ma_length=ma_len)
        else:
            mm = {}  # 비활성 → 빈 맵 (skip 안 됨)

        tasks = [(tk, tkm_map.get(tk,"?"), cc[tk], base_dict, bcfg, mm, mf_enabled) for tk in cc]
        trades = []
        ...
        total = len(trades)
        if total == 0:
            print(f"{name:<6} {desc:<16} | NO TRADES"); continue
        ...
        print(f"{name:<6} {desc:<16} | {total:>4} {pf:>6.2f} {sum(pnl_list):>+12,.0f} {per_trade:>+8,.0f} | "
