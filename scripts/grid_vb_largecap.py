"""scripts/grid_vb_largecap.py — 변동성 돌파 전략 80조합 그리드 (KOSPI 대형주).

grid_volatility_breakout.py 와 동일 파라미터·선정 기준,
...
결과: reports/vb_largecap_grid_result.md

사용:
    python -u scripts/grid_vb_largecap.py
    python -u scripts/grid_vb_largecap.py --verify
"""
                ...
                for tp_mode in TP_MODE_VALS:
                    for vc in VOL_CONF_VALS:
                        if tp_mode == "none_trail":
                            tp_pct, use_trailing, trail_pct = 0.0, True, 0.02
                        else:
                            tp_pct, use_trailing, trail_pct = 0.03, False, 0.02
...
def _run_vb_grid(combos: list[dict], cache: GridCache) -> list[dict]:
    cache.prepare_bytes()
    ...
    results: list[dict] = []
    t0 = _time.time()
    print(f"[VB GRID LARGECAP] {n}조합 × {len(cache.candles)}종목  workers={n_workers}", flush=True)
    ...
    try:
        ...
        with _PPE(max_workers=n_workers, mp_context=ctx) as ex:
            ...
            if _use_tqdm:
                it = _tqdm(it, total=n, desc="vb largecap", unit="combo")
...
def _write_report(
    ...
    out = Path("reports/vb_largecap_grid_result.md")
...
async def main() -> None:
    import argparse
    parser = argparse.ArgumentParser(description="변동성 돌파 80조합 그리드 (대형주)")
