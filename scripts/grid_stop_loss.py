"""scripts/grid_stop_loss.py — stop_loss 파라미터 그리드."""
...
from core.indicators import calculate_atr_trailing_stop, calculate_atr_stop_loss, get_latest_atr
...
# (name, desc, atr_stop_enabled, atr_mult, atr_min, atr_max, fixed_pct)
SCENARIOS = [
    ("S0", "ATR x1.5 max8% (현재)", True, 1.5, 0.015, 0.080, None),
    ("S1", "ATR x1.0 max8%",        True, 1.0, 0.015, 0.080, None),
    ("S2", "ATR x1.5 max5%",        True, 1.5, 0.015, 0.050, None),
    ("S3", "ATR x1.0 max5%",        True, 1.0, 0.015, 0.050, None),
    ("S4", "고정 -3%",               False, 0, 0, 0, -0.03),
    ("S5", "고정 -5%",               False, 0, 0, 0, -0.05),
    ("S6", "ATR x2.0 max10%",       True, 2.0, 0.015, 0.100, None),
]


def calc_stop(entry_price, ticker, as_of, sc):
    """시나리오별 stop_loss 계산."""
    name, desc, atr_enabled, atr_mult, atr_min, atr_max, fixed_pct = sc
    if not atr_enabled:
        return entry_price * (1 + fixed_pct)
    atr_pct = get_latest_atr(DB_PATH, ticker, as_of)
    if atr_pct is None:
        return entry_price * 0.92  # fallback -8%
    return calculate_atr_stop_loss(entry_price, atr_pct, atr_mult, atr_min, atr_max)


def run_f_day(day_candles, strategy, costs, tcfg, ticker, sc):
    ...
    for idx, row in candles.iterrows():
        ...
        if position is None:
            ...
            if sig and sig.side == "buy":
                ...
                sl = calc_stop(ep, ticker, as_of, sc)
...
def run_multi(ticker, all_candles, tcfg, bcfg, tkm, mmap, sc):
    ...
    for date, dc in df.groupby("date"):
        ...
        dt = run_f_day(dd, strat, costs, tcfg, ticker, sc)
...
def _worker(args):
    tk, tkm, cp, tcfg, bcfg, mm, sc = args
    return tk, run_multi(tk, pickle.loads(cp), tcfg, bcfg, tkm, mm, sc)
...
async def main():
    ...
    print(f"Stop loss grid")
    print(f"Tickers: {len(cc)}, Workers: {w}")
    print()
    hdr = f"{'name':<5} {'desc':<22} | {'n':>4} {'PF':>6} {'PnL':>12} | {'sl':>4} {'ts':>4} {'fc':>4} | {'occ':>4} {'DD':>10} | {'강세':>5} {'횡보':>5} {'약세':>5}"
    print(hdr)
    print("-"*len(hdr))

    for sc in SCENARIOS:
        name = sc[0]
        tasks = [(tk, tkm.get(tk,"?"), cc[tk], cfg, bcfg, mm, sc) for tk in cc]
        trades = []
        ...
        total = len(trades)
        if total == 0:
            print(f"{name:<5} {sc[1]:<22} | NO TRADES")
            continue
        ...
        print(f"{name:<5} {sc[1]:<22} | {total:>4} {pf:>6.2f} {sum(pnl_list):>+12,.0f} | "
